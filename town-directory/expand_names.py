...
with open('roster_generator.php', 'r', encoding='utf-8') as f:
    content = f.read()

# Replace getFirstNames pools
start_first = content.find("    $pools = [\\n        'Human'")
if start_first == -1:
    start_first = content.find("    $pools = [\n        'Human'")

end_first = content.find("    // Default fallback for unknown/custom races", start_first)

# Replace getSurnames pools
start_sur = content.find("    $pools = [\n        'Human'", end_first)
if start_sur == -1:
    start_sur = content.find("    $pools = [\\n        'Human'", end_first)

end_sur = content.find("    // Handle unknown races: try partial match", start_sur)

new_content = content[:start_first] + pools_first + "\n\n" + content[end_first:start_sur] + pools_sur + "\n\n" + content[end_sur:]

with open('roster_generator.php', 'w', encoding='utf-8') as f:
    f.write(new_content)

print("Updated roster_generator.php successfully.")
